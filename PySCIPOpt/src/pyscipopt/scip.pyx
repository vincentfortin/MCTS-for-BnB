...
cdef class Model:
    ...
    def branchVarUp2(self, variable):
        """Branch on a non-continuous variable.
        ...
        PY_SCIP_CALL(SCIPbranchVar(self._scip, (<Variable>variable).scip_var, NULL, NULL, &upchild))
        return Node.create(upchild)


    def branchVarDown2(self, variable):
        """Branch on a non-continuous variable.
        ...
        PY_SCIP_CALL(SCIPbranchVar(self._scip, (<Variable>variable).scip_var, &downchild, NULL, NULL))
    ...
    def getState(self, prev_state = None):
        ...
        cdef np.ndarray[np.int32_t,   ndim=1] col_nlocks_up
        cdef np.ndarray[np.int32_t,   ndim=1] col_nlocks_down
        ...
        if not update:
            ...
            col_nlocks_up    = np.empty(shape=(ncols, ), dtype=np.int32)
            col_nlocks_down  = np.empty(shape=(ncols, ), dtype=np.int32)
        else:
            ...
            col_nlocks_up    = prev_state['col']['nlocksup']
            col_nlocks_down  = prev_state['col']['nlocksdown']
        ...
        for i in range(ncols):
            ...
            # Number of variable locks
            col_nlocks_up[col_i] = SCIPvarGetNLocksUp(var)
            col_nlocks_down[col_i] = SCIPvarGetNLocksDown(var)
        ...
        return {
            'col': {
                ...
                'nlocksup':     col_nlocks_up,
                'nlocksdown':   col_nlocks_down,
            },
            'row': {
                'lhss':          row_lhss,
                'rhss':          row_rhss,
    ...
    def getDivingState(self, candidates):
        cdef SCIP* scip = self._scip
        ...
        cdef np.ndarray[np.int32_t, ndim=1] cand_nvar_lock_up
        cdef np.ndarray[np.int32_t, ndim=1] cand_nvar_lock_down
        cdef np.ndarray[np.float32_t, ndim=1] cand_solfracs
        ...
        cand_nvar_lock_up        = np.empty(shape=(ncands, ), dtype=np.int32)
        cand_nvar_lock_down      = np.empty(shape=(ncands, ), dtype=np.int32)
        ...
        for cand_i in range(ncands):
            ...
            cand_nvar_lock_up[cand_i] = SCIPvarGetNLocksUp(var)
            cand_nvar_lock_down[cand_i] = SCIPvarGetNLocksDown(var)
        ...
        return {
            ...
            'n_locks_up':           cand_nvar_lock_up,
            'n_locks_down':         cand_nvar_lock_down,
        }
    ...
    def getKhalilState(self, root_info, candidates):
        ...
        if SCIPgetNNodes(scip) == 1:
            ...
            root_info['col']['root_nvar_lock_up']   = {}
            root_info['col']['root_nvar_lock_down'] = {}
            ...
            for i in range(ncols):
                ...
                root_info['col']['root_nvar_lock_up'][col_i]   = SCIPvarGetNLocksUp(var)
                root_info['col']['root_nvar_lock_down'][col_i] = SCIPvarGetNLocksDown(var)
        ...
        for cand_i in range(ncands):
            ...
            cand_root_ncoefs_min[cand_i]   = root_info['col']['root_ncoefs_min'][col_i]
            cand_root_ncoefs_max[cand_i]   = root_info['col']['root_ncoefs_max'][col_i]
            cand_nvar_lock_up[cand_i]      = root_info['col']['root_nvar_lock_up'][col_i]
            cand_nvar_lock_down[cand_i]    = root_info['col']['root_nvar_lock_down'][col_i]
    ...
    # def inclGuidediving(self):
    #     PY_SCIP_CALL(SCIPincludeHeurGuideddiving(self._scip))


    def getDiveSets(self):
        # cdef SCIP_HEUR _heur
        # cdef SCIP_DIVESET ** divesets
        PY_SCIP_CALL(SCIPincludeHeurGuideddiving(self._scip))
        cdef SCIP_HEURDATA* heurdata;
        cdef SCIP_DIVESET* diveset;

        heur = SCIPfindHeur(self._scip,'guideddiving')

        heurdata = SCIPheurGetData(heur);
        diveset = SCIPheurGetDivesets(heur)[0];
        print(dereference(diveset).keys())
